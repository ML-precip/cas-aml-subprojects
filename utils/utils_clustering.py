...
def KCluster(cldat, data, N):
    ...
    labels = pd.DataFrame(kmeans.labels_, index=np.array(
        data['time']), columns=['cluster'])
...
def prepare_cluster_data(day_clusters, mvar):
    ...
    for i_clus in range(len(day_clusters)):
        ...
        tmp_df = pd.DataFrame(
            {'date': tmp['time'], mvar: tmp[mvar], 'Cluster': i_clus})
...
def visualize_cluster(myclusters, nfreq, N):
    ...
    for i in range(0, N):
        cs = axs[i].contourf(myclusters.lon, myclusters.lat,
                             myclusters['z'][i, :, :],
                             cmap=plt.cm.RdBu_r,
